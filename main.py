...
from mlFunc import *
def load_and_plot_histograms(file_path):
    # Load the embedding pairs from the file
    embeddings = numpy.loadtxt(file_path, delimiter=',', usecols=range(10))
    labels = numpy.loadtxt(file_path, delimiter=',', usecols=-1, dtype=int)

    # Separate embeddings based on class labels
    ...
    for dim in range(embeddings.shape[1]):
        ...
        # Plot histogram for same speaker embeddings
        hist_same, bin_edges_same, _ = ax.hist(same_speaker_embeddings[:, dim], bins=40, alpha=0.5,
                                               label='Same Speaker')

        # Plot histogram for different speaker embeddings
        hist_diff, bin_edges_diff, _ = ax.hist(diff_speaker_embeddings[:, dim], bins=40, alpha=0.5,
...
def load_and_show_scatter_plots(file_path):
    # Load the embedding pairs from the file
    embeddings = numpy.loadtxt(file_path, delimiter=',', usecols=range(10))
    labels = numpy.loadtxt(file_path, delimiter=',', usecols=-1, dtype=int)
    ...
    # Show scatter plots for each pair of dimensions
    for dim1 in range(embeddings.shape[1]):
        for dim2 in range(dim1+1, embeddings.shape[1]):
            fig, ax = plt.subplots()

            # Scatter plot for same speaker embeddings
            ax.scatter(same_speaker_embeddings[:, dim1], same_speaker_embeddings[:, dim2], color='blue', label='Same Speaker')

            # Scatter plot for different speaker embeddings
            ax.scatter(diff_speaker_embeddings[:, dim1], diff_speaker_embeddings[:, dim2], color='red', label='Different Speaker')
...
if __name__ == "__main__":
    ...
    #load_and_plot_histograms("Train.txt")
    load_and_show_scatter_plots("Train.txt")
